...
addpath(genpath('../CenteredAligment'));
...
for b=1:9
    ...
    parfor i = 1:30
        ...
        % Alignment
        mu = CenteredAligment(Xtr, Ytr+1);
        Ytr(Ytr == 0) = -1;
        aux1 = zeros(length(tte),2);
        for m = 1:R
            hyp = minimize(hyp0,'gp', -Ncg, inf, mean1, cov, lik, Xtr, Ytr(:,m));
            [~, ~, ~, ~, aux] = gp(hyp, inf, mean1, cov, lik, Xtr,...
                                    Ytr(:,m), Xte, ones(length(tte), 1));
            aux = [1-exp(aux), exp(aux);];
            aux1 = aux1 + aux*mu(m);
        end
        aux1 = aux1/sum(mu);
        predtest = aux1(:,2);
    ...
    fileSave = strcat('ResultsMe2/', name);
